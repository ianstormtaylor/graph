...
exports.update = function (graph, callback) {
  if ('object' != type(graph)) return callback(new Error('invalid graph'));
  if ('string' != type(graph.id)) return callback(new Error('id required'));
  var id = graph.id;
  var data;
  ...
  db.run(function (t) {
    exec(t, 'UPDATE todos SET DATA=? WHERE ID=?', [data, id], function (err, res) {
      if (err) return callback(err);
      callback(null, graph);
    });
  });
